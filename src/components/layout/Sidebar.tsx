...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
    ...
    const additionalLinks = [
        { name: t('sidebar.wiki'), icon: <Book size={18} />, path: '#' },
        { name: t('sidebar.rules'), icon: <FileText size={18} />, path: '#' },
        { name: t('sidebar.history'), icon: <History size={18} />, path: '#' },
            ...
            <div className={sidebarClasses}>
                <div className="flex flex-col h-full bg-nr-bg/40">
                    <div className="h-16 flex items-center justify-between px-4 border-b border-nr-border">
                        <span className="font-serif font-bold text-xl text-nr-text">{t('sidebar.menu')}</span>
                    ...
                    <div className="flex-1 overflow-y-auto py-4">
                        <nav className="flex flex-col gap-1 px-3 mb-8 md:hidden">
                            <Link to="/" onClick={onClose} className="px-3 py-2 text-nr-text hover:bg-nr-border/50 rounded-md font-medium">{t('header.home')}</Link>
                            <Link to="/roster" onClick={onClose} className="px-3 py-2 text-nr-text hover:bg-nr-border/50 rounded-md font-medium">{t('header.roster')}</Link>
                            <Link to="/events" onClick={onClose} className="px-3 py-2 text-nr-text hover:bg-nr-border/50 rounded-md font-medium">{t('header.events')}</Link>
                            {hasAdminPermission && (
                                <Link to="/admin" onClick={onClose} className="px-3 py-2 text-nr-text hover:bg-nr-border/50 rounded-md font-medium">{t('header.admin')}</Link>
                            )}
                        </nav>

                        <div className="px-4 mb-2 text-xs font-semibold uppercase text-nr-text/50 tracking-wider">{t('sidebar.additional')}</div>

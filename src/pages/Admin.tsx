...
const Admin: React.FC = () => {
    ...
    const tabs = [
        { id: 'users', label: t('admin.tabs.users', 'Користувачі'), icon: <Users size={18} /> },
        { id: 'news', label: t('admin.tabs.news', 'Редактор Новин'), icon: <FileText size={18} /> },
        { id: 'settings', label: t('admin.tabs.settings', 'Налаштування'), icon: <Settings size={18} /> },
    ];
    ...
    return (
        <div className="flex-1 flex flex-col md:flex-row h-full pt-16 animate-fade-in-up">
            ...
            <aside className="w-full md:w-64 glass border-r border-nr-border md:h-[calc(100vh-4rem)] md:sticky top-16 z-10">
                <div className="p-6">
                    <h2 className="font-serif font-bold text-xl text-nr-text mb-6">{t('admin.title', 'Адмін Панель')}</h2>
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    {activeTab === 'users' && (
                        <div className="space-y-6">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                                <div>
                                    <h1 className="font-serif text-3xl font-bold text-nr-text">{t('admin.users.title', 'Управління Користувачами')}</h1>
                                    <p className="text-nr-text/60 mt-1">{t('admin.users.subtitle', 'Перегляд та редагування особового складу')}</p>
                                </div>
                                <Button className="flex items-center gap-2"><Plus size={16} /> {t('admin.users.add', 'Додати бійця')}</Button>
                            </div>
                            ...
                            <div className="glass-card rounded-xl shadow-sm border border-nr-border overflow-hidden">
                                <div className="p-4 border-b border-nr-border flex justify-between items-center bg-black/5 dark:bg-white/5">
                                    <div className="relative">
                                        ...
                                        <input
                                            type="text"
                                            placeholder={t('admin.users.search', 'Пошук...')}
                                    ...
                                    <div className="flex gap-2">
                                        <select className="bg-nr-surface border border-nr-border rounded-md px-3 py-2 text-sm text-nr-text focus:outline-none focus:border-nr-accent">
                                            <option>{t('admin.users.filter_all', 'Всі звання')}</option>
                                            <option>{t('admin.users.filter_officers', 'Офіцери')}</option>
                                            <option>{t('admin.users.filter_privates', 'Рядові')}</option>
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="bg-black/5 dark:bg-white/5 text-nr-text/60 text-sm uppercase tracking-wider">
                                                <th className="p-4 font-semibold">{t('admin.users.table.name', 'Ім\'я граця')}</th>
                                                <th className="p-4 font-semibold">{t('admin.users.table.rank', 'Звання')}</th>
                                                <th className="p-4 font-semibold">{t('admin.users.table.status', 'Статус')}</th>
                                                <th className="p-4 font-semibold text-right">{t('admin.users.table.actions', 'Дії')}</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-nr-border">
                                            {[1, 2, 3, 4, 5].map(i => (
                                                <tr key={i} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                                                    ...
                                                    <td className="p-4 text-nr-text">{t('roster.member.recruit', 'Канонір')}</td>
                                                    <td className="p-4">
                                                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20">
                                                            ...
                                                            {t('roster.status.active', 'Активний')}
                                                        </span>
                                                    </td>
                                                    <td className="p-4">
                                ...
                                <div className="p-4 border-t border-nr-border text-sm text-nr-text/50 flex justify-between items-center bg-black/5 dark:bg-white/5">
                                    <span>{t('admin.users.pagination.showing', 'Показано')} 1-5 {t('admin.users.pagination.out_of', 'з')} 42</span>
                                    <div className="flex gap-1">
                                        <button className="px-3 py-1 border border-nr-border rounded bg-nr-surface text-nr-text disabled:opacity-50">{t('admin.users.pagination.prev', 'Поп')}</button>
                                        <button className="px-3 py-1 border border-nr-border rounded bg-nr-surface text-nr-text">{t('admin.users.pagination.next', 'Нас')}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {activeTab === 'news' && (
                        <div className="space-y-6">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                                <div>
                                    <h1 className="font-serif text-3xl font-bold text-nr-text">{t('admin.news.title', 'Редактор Новин')}</h1>
                                    <p className="text-nr-text/60 mt-1">{t('admin.news.subtitle', 'Створення та публікація нових записів')}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                <div className="lg:col-span-2 space-y-6">
                                    ...
                                    <div className="glass-card rounded-xl p-6 shadow-sm border border-nr-border">
                                        <input
                                            type="text"
                                            placeholder={t('admin.news.editor.title_placeholder', 'Заголовок новини')}
                                        ...
                                        <div className="flex gap-2 mb-4 border-b border-nr-border pb-4">
                                            ...
                                            <button className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded text-nr-text flex items-center gap-1 text-sm"><Edit3 size={16} /> {t('admin.news.editor.description', 'Опис')}</button>
                                        </div>

                                        <textarea
                                            placeholder={t('admin.news.editor.content_placeholder', 'Почніть писати текст новини тут...')}
                                ...
                                <div className="space-y-6">
                                    {/* Sidebar options */}
                                    <div className="glass-card rounded-xl p-6 shadow-sm border border-nr-border">
                                        <h3 className="font-bold text-nr-text mb-4 border-b border-nr-border pb-2">{t('admin.news.publish.title', 'Публікація')}</h3>
                                        <div className="space-y-4">
                                            <div>
                                                <label className="block text-sm font-medium text-nr-text/70 mb-1">{t('admin.news.publish.category', 'Категорія')}</label>
                                                <select className="w-full bg-nr-surface border border-nr-border rounded-md px-3 py-2 focus:outline-none focus:border-nr-accent text-nr-text">
                                                    <option>{t('events.types.battle', 'Битва')}</option>
                                                    <option>{t('events.types.training', 'Тренування')}</option>
                                                    <option>{t('admin.news.publish.category_important', 'Важливе')}</option>
                                                </select>
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-nr-text/70 mb-1">{t('admin.news.publish.cover', 'Обкладинка (URL)')}</label>
                                            ...
                                            <div className="pt-4 flex flex-col gap-2">
                                                <Button variant="primary" className="w-full flex items-center justify-center gap-2"><CheckCircle size={18} /> {t('admin.news.publish.publish_btn', 'Опублікувати')}</Button>
                                                <Button variant="secondary" className="w-full flex items-center justify-center gap-2"><Save size={18} /> {t('admin.news.publish.draft_btn', 'Зберегти чернетку')}</Button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    {activeTab === 'settings' && (
                        <div className="space-y-6">
                            <div>
                                <h1 className="font-serif text-3xl font-bold text-nr-text">{t('admin.settings.title', 'Налаштування Системи')}</h1>
                                <p className="text-nr-text/60 mt-1">{t('admin.settings.subtitle', 'Загальні параметри порталу')}</p>
                            </div>
                            <div className="glass-card rounded-xl p-8 border border-nr-border shadow-sm text-center py-20">
                                ...
                                <h3 className="text-xl font-bold text-nr-text mb-2">{t('admin.settings.wip', 'Налаштування в розробці')}</h3>
                                <p className="text-nr-text/60 max-w-md mx-auto">{t('admin.settings.wip_desc', 'Модуль глобальних налаштувань буде додано в наступному оновленні системи.')}</p>
                            </div>
                        </div>
                    )}
